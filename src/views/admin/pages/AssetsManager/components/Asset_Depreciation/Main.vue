<template lang="">
    <Create_Asset ref="Create_Modal" @ReloadData="ReloadData" :assetId="assetId" :bravo_asset_code="bravo_asset_code"></Create_Asset>
    <Delete_Asset ref="Delete_Modal" @ReloadData="ReloadData" :assetId="assetId"></Delete_Asset>
    <Update_Asset ref="Update_Modal" @ReloadData="ReloadData" :assetId="assetId" :bravo_asset_code="bravo_asset_code"></Update_Asset>
                ...
                <div class="w-100 border-0 p-3" type="button" data-bs-toggle="collapse" :data-bs-target="'#'+slug" aria-expanded="true">
                    <div class="d-flex">
                        <div class="d-flex align-items-center gap-3 flex-grow-1">
                            ...
                            <div>
                                <h6 class="mb-0 fw-bold text-primary mb-1">Khấu hao tài sản</h6>
                                <p class="mb-0 text-muted" style="font-size:12px">Danh sách Khấu hao tài sản</p>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <table v-if="data && data.length != 0" class="table table-sm table-striped table-fixed align-middle">
                        <thead>
                            <tr>
                                ...
                                <th class="py-2">Tên khấu hao tài sản</th>
                                <th class="py-2">Giá trị khấu hao tài sản</th>
                        ...
                        <tbody>
                            <tr  v-for="(item, index) in data" :key="index">
                                ...
                                <td class="fw-bold text-back-50 text-capitalize">{{item.asset_depreciation_content}}</td>
                                <td>{{FormatCurrency(item.asset_depreciation_value)}}</td>
                                <td>
                                ...
                                <td>
                                    <div class="d-flex gap-1 justify-content-end">
                                        ...
                                        <button v-if="!view" @click="$refs.Update_Modal._Init({id:item.id})" type="button" class="btn btn-info bg-gradient btn-sm p-1">
