<template lang="">
    ...
    <div class="accordion" :id="slug+'Parent'">
        <div class="accordion-item mb-3 rounded-0 border-bottom position-relative">
            ...
            v-if="$CheckPermission('Asset_Repair','is_allow_create')"
            >
                <button
                v-if="!view"
                ...
                <div class="w-100 border-0 p-3" type="button" data-bs-toggle="collapse" :data-bs-target="'#'+slug" aria-expanded="true">
                    <div class="d-flex">
                        <div class="d-flex align-items-center gap-3 flex-grow-1">
                            ...
                            <div>
                                <h6 class="mb-0 fw-bold text-primary mb-1">Sửa chữa tài sản</h6>
                                <p class="mb-0 text-muted" style="font-size:12px">Danh sách sửa chữa tài sản</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div :id="slug" class="accordion-collapse collapse " :data-bs-parent="'#'+slug+'Parent'">
                ...
                v-if="$CheckPermission('Asset_Repair','is_allow_access')"
                >
                    <table v-if="data && data.length != 0" class="table table-sm table-striped table-fixed align-middle">
                        <thead>
                            <tr>
                                ...
                                <th class="py-2">Tên pháp lý đất</th>
                                <th class="py-2">Giá trị sửa chữa</th>
                        ...
                        <tbody>
                            <tr  v-for="(item, index) in data" :key="index">
                                ...
                                <td class="fw-bold text-back-50 text-capitalize">{{item.asset_repair_content}}</td>
                                <td>{{FormatCurrency(item.asset_repair_value)}}</td>
                                <td>
                                ...
                                <td>
                                    <div class="d-flex gap-1 justify-content-end">
                                        <div v-if="$CheckPermission('Asset_Repair','is_allow_edit')">
                                        ...
                                        <div v-if="$CheckPermission('Asset_Repair','is_allow_delete')">
