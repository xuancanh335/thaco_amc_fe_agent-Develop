        ...
        <div class="bg-white p-5 rounded-3" style="width:1000px">
            <div class="d-flex mb-4">
                <div class="d-flex align-items-center gap-3 flex-grow-1">
                    ...
                    <div>
                        <h6 class="mb-0 fw-bold text-primary mb-1">Tạo hợp đồng và hồ sơ</h6>
                        <p class="mb-0 text-muted" style="font-size:12px">Tạo hợp đồng và hồ sơ với dữ liệu đi kèm</p>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-lg-6">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" placeholder="Nhập giá trị hợp đồng ..." v-model.lazy="data.contract_value" v-number="number">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        <label class="form-label">Dữ liệu đồng bộ</label>
                        <DropdownTable 
                            ref="DropdownTable" 
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                ...
                <button type="button" class="btn btn-success bg-gradient" @click="CreateLegal">Tạo hồ sơ</button>
            </div>
        </div>
    </div>
</template>
    ...
    methods : {
        ...
        async CreateLegal(){ 
            try {
                ...
                axios.post(PATH_API_DATA + '/project-contract/create',params,config)
                .then(async res => {
                    await this.$refs.FileManager.UpdateLegal({id:res.data.data.id,type:'TYPE_PROJECT_CONTRACT'})
                    this.Display(false)
        ...
        GetDataSync(){
            ...
            .then(res => {
                ...
                this.list_sync.data = data
                // this.list_sync.total = data.total
                let sync_data= res.data.data.items
                this.list_sync.total = res.data.data.total
                this.list_sync.data = sync_data.map(item => {
                    let arr = {
                        value : item.docNo,
