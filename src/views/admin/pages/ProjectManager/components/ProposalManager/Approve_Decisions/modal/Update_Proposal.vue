        ...
        <div class="bg-white p-5 rounded-3" style="width:1000px">
            <div class="d-flex mb-4">
                <div class="d-flex align-items-center gap-3 flex-grow-1">
                    ...
                    <div>
                        <h6 class="mb-0 fw-bold text-gradient-blue mb-1">Cập nhật quyết định phê duyệt</h6>
                        <p class="mb-0 text-muted" style="font-size:12px">Cập nhật quyết định phê duyệt với dữ liệu đi kèm</p>
                    </div>
                </div>
    ...
    methods : {
        async UpdateLegal(){            
            try {
                ...
                axios.post(apiProject + '/proposal-approve-decisions/update',params,config)
                .then(async res => {
                    this.Display(false)
        ...
        async GetDataLegal(){            
            try {
                ...
                const config = {headers: {Authorization: `Bearer ${authorize.token}`}};
                axios.post(apiProject + '/proposal-approve-decisions/get-by-id',params,config)
                .then(async res => {
        ...
        async Display(value,data){
            ...
            this.id = await data
            this.$refs.FileManager._Init({id:data,type:'TYPE_APPROVE_DECISIONS'});
