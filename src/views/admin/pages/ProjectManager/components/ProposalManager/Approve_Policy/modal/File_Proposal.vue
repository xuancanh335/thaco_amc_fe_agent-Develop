        ...
        <div class="bg-white p-4 rounded-3" style="width:600px; max-height:650px; overflow:hidden">
            ...
            <div class="d-flex mb-4">
                <div class="d-flex align-items-center gap-3 flex-grow-1">
                    ...
                    <div>
                        <h6 class="mb-0 fw-bold text-gradient-blue mb-1">Danh sách tập tin phê duyệt chủ trương đầu tư</h6>
                        <p class="mb-0 text-muted" style="font-size:12px">Danh sách tập tin phê duyệt chủ trương đầu tư với dữ liệu đi kèm</p>
                    </div>
                </div>
...
export default {
    ...
    methods : {
        ...
        async UpdateLegal(){           
            ...
            data.append('object_type', 'TYPE_APPROVE_POLICY');
            for (let i = 0; i < this.files.length; i++) {
                data.append('file', this.files[i]);
            }
            let config = {
                method: 'post',
                url: API_PROJECT_PATH + '/file-manager/create',
                headers: { 
        ...
        async RemoveFile(id){            
            try {
                ...
                axios.post(API_PROJECT_PATH + '/file-manager/delete',params,config)
                .then(async res => {
                    this.GetDataFile()
        ...
        async GetDataFile(){            
            try {
                const params = {
                    request: {
                        object_id   : this.id,
                        object_type : "TYPE_APPROVE_POLICY"
                    },
                    page        : 0,
                    limit       : 0,
                    tenant_id   : authorize.tenant_id,
                ...
                const config = {headers: {Authorization: `Bearer ${authorize.token}`}};
                axios.post(API_PROJECT_PATH + '/file-manager/get-all',params,config)
                .then(async res => {
