<template lang="">
    ...
    <div class="accordion" :id="slug+'Parent'">
        <div class="accordion-item mb-3 rounded-0 border-bottom position-relative">
            ...
            v-if="$CheckPermission('Proposal_Implement_Project','is_allow_create')"
            >
                ...
                <div class="w-100 border-0 p-3" type="button" data-bs-toggle="collapse" :data-bs-target="'#'+slug" aria-expanded="true">
                    <div class="d-flex">
                        <div class="d-flex align-items-center gap-3 flex-grow-1">
                            ...
                            <div>
                                <h6 class="mb-0 fw-bold text-gradient-blue mb-1">Phê duyệt chủ trương thực hiện dự án</h6>
                                <p class="mb-0 text-muted" style="font-size:12px">Danh sách chủ trương thực hiện dự án</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div :id="slug" class="accordion-collapse collapse " :data-bs-parent="'#'+slug+'Parent'">
                <div class="accordion-body border-top" v-if="$CheckPermission('Proposal_Implement_Project','is_allow_access')">
                    <table v-if="data && data.length != 0" class="table table-lg table table-striped align-middle">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in data" :key="index">
                                ...
                                <td class="fw-bold text-back-50 text-capitalize">{{item.content}}</td>                               
                                <td>
                                    <span>{{item.description}}</span>
                                </td>
                                <td>{{FormatCurrency(item.content_value)}}</td>
                                <td>
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <div v-if="$CheckPermission('Proposal_Implement_Project','is_allow_edit')">
                                        ...
                                        <div v-if="$CheckPermission('Proposal_Implement_Project','is_allow_delete')">
