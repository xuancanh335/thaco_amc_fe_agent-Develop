        ...
        <div class="bg-white p-5 rounded-3" style="width:700px">
            <div class="d-flex mb-4">
                <div class="d-flex align-items-center gap-3 flex-grow-1">
                    ...
                    <div>
                        <h6 class="mb-0 fw-bold text-gradient-blue mb-1">Tạo tài sản mới</h6>
                        <p class="mb-0 text-muted" style="font-size:12px">Tạo tài sản với dữ liệu đi kèm</p>
                    </div>
                </div>
            ...
            <div>
                <div class="row">
                    ...
                    <div class="col-lg-6">
                        <div class="mb-3">
                            ...
                            <select class="form-select rounded-0 border-0 border-bottom shadow-none" v-model="data.tenant_used_id">
                                <option value="" selected>--CHỌN ĐƠN VỊ SỬ DỤNG--</option>
                    ...
                    <div class="col-lg-6">
                        <div class="mb-3">
                            <label class="form-label mb-0">Ngày tạo tài sản (<span class="text-danger">*</span>)</label>
            ...
            <div class="d-flex justify-content-end gap-2 mt-4">
                ...
                <button type="button" class="btn btn-success bg-gradient" @click="CreateAsset">Tạo tài sản</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    data(){
        return {
            ...
            data : {
                status_content : '',
                // asset_code                  : "TAISAN1",
                // asset_name                  : "Tài sản 1",
                // asset_parent                : "Tài sản 0",
                // asset_type                  : "Tài sản 1",
                // total_unit_price            : "10000000000",
                // total_depreciation_price    : "10000000",
                // total_area_used             : "100",
                // asset_create_time           : "2023-10-11T04:25:00.261Z",
                // description                 : "Tài sản 1",
                tenant_used_id : '',
    ...
    methods : {
        async CreateAsset(){ 
            try {
                var project_id = document.getElementById('Modal_UpdateProject').dataset.id;
                const params = this.data
                params.project_id = project_id
                const config = {headers: {Authorization: `Bearer ${authorize.token}`}};
                axios.post(apiProject + '/asset/create',params,config)
                .then(async res => {
                    this.Display(false)
                    this.$emit('ReloadData');
                    ShowToast({status_code : 200, message : 'Tạo tài sản thành công'});
                })
                .catch(err => {
