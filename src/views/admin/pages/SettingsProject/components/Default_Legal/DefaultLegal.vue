        ...
        <div class="card card-one">
            <Filters @handleFilter="setDataFilter" ref="Filter" :date_time="false">
                <nav class="nav nav-icon nav-icon-sm ms-auto">
                    <button data-bs-toggle="modal" data-bs-target="#Modal_CreateDefaultLegal" class="btn bg-success text-white rounded-circle p-2 border-0 hover-jello" href="#">
                ...
                <table class="table table-striped align-middle">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" width="10%">Loại pháp lý</th>
                            <th scope="col" width="15%">Nội dung pháp lý</th>
                            <th scope="col" class="text-center" width="8%">Vị trí hiển thị</th>
                            <th scope="col" class="text-center" width="10%">Trạng thái kích hoạt</th>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in data" :key="index">
                            ...
                            <td>
                                <span v-if="item.default_type == 0">Pháp lý quy hoạch</span>
                                <span v-if="item.default_type == 1">Pháp lý đầu tư</span>
                                <span v-if="item.default_type == 2">Pháp lý đất</span>
                                <span v-if="item.default_type == 3">Pháp lý xây dựng</span>
                                <span v-if="item.default_type == 4">Pháp lý nghiệm thu & sở hữu công trình</span>
                            </td>
                            <td>{{item.default_content}}</td>
                            <td>
                                <div class="d-flex justify-content-center">
                                    <span class="bg-gradient-blue d-block d-flex align-items-center justify-content-center rounded-circle text-white" style="width:30px;height:30px; font-size:12px">{{item.default_stt}}</span>
                                </div>
                            </td>
                            ...
                            <td >
                                <div class="d-flex gap-1">
                                    ...
                                    @click="OpenModal('Modal_UpdateDefaultLegal',item.id)">
...
export default {
    ...
    methods : {
        ...
        getData(){
            ...
            axios({
                method:'post',
                url: PATH_API_USER + '/default-legal/get-all',
                headers: {
