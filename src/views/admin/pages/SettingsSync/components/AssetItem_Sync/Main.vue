<template lang="">
    <div class="tab-pane h-100 fade show" id="asset-item-sync-pane" role="tabpanel">
        ...
        <div class="card card-one">
            ...
            <div class="card-body d-flex flex-column position-relative">
                ...
                <div class='table-responsive h-scrollbar mb-3 flex-grow-1'>
                    <table class="table table-striped align-middle table-fixed" style="width:200%">
                        <thead>
                            <tr>
                                <th class="text-center" scope="col" width="2%">STT</th>
                                <th scope="col" width="2%">Mã tài sản</th>
                                <th scope="col" width="2%">Stt tài sản</th>
                                <th scope="col" width="5%">Tên tài sản</th>
                                <th scope="col" width="1%">Đvt</th>
                                <th scope="col" width="3%">Mã nhóm phân loại</th>
                                <th scope="col" width="3%">Ngày tăng</th>
                                <th scope="col" width="3%">Ngày bắt đầu sử dụng</th>
                                <th scope="col" width="1%" class="text-nowrap">Ngày bắt đầu khấu hao</th>
                                <th scope="col" width="1%">Nguyên giá</th>
                                <th scope="col" width="1%" class="text-nowrap">Số kỳ khấu hao</th>
                                <th scope="col" width="2%">Tài khoản TS</th>
                                <th scope="col" width="3%">Tài khoản khấu hao</th>
                                <th scope="col" width="3%">Tài khoản chi phí</th>
                                <th scope="col" width="5%">Mã sản phẩm công trình</th>
                                <th scope="col" width="3%">Khoản mục phí</th>
                                <th scope="col" width="2%"><IconDots :size="20" stroke-width="2" class="text-muted" /></th>
                            </tr>
                        </thead>
                        <tbody v-if="$CheckPermission('Sync_Bravo_Config','is_allow_access')">
                            <tr v-for="(item, index) in data" :key="index">
                                ...
                                <td>{{FormatDate(item.firstUsedDate,'DD/MM/YYYY')}}</td>
                                <td>{{FormatDate(item.firstDeprDate,'DD/MM/YYYY')}}</td>
                                <td>{{item.originalCost}}</td>
                                <td>{{item.usefulMonth}}</td>
                                <td>{{item.assetAccount}}</td>
                                <td>{{item.deprDebitAccount}}</td>
                                <td>{{item.deprCreditAccount}}</td>
                                <td>{{item.productCostId}}</td>
...
export default {
    ...
    methods : {
        ...
        SyncData(data){
            ...
            let config = {headers: {Authorization: `Bearer ${this.authorize.token}`}}
            axios.post(PATH_API_WEBHOOK + '/asset-item/sync-data-bravo',params,config)
            .then(res => {
                setTimeout(() => {
