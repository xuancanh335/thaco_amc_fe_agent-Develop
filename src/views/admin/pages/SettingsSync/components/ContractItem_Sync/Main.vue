<template lang="">
    <div class="tab-pane h-100 fade show" id="contract-item-sync-pane" role="tabpanel">
                ...
                <table class="table table-striped align-middle" style="width:140%">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" width="8%">Số hợp đồng</th>
                            <th scope="col" width="8%">Mã dự án</th>
                            <th scope="col" width="25%">Nội dung</th>
                            <th scope="col" width="15%">Giá trị hợp đồng</th>
                    ...
                    <tbody v-if="$CheckPermission('Sync_Bravo_Config','is_allow_access')">
                        <tr v-for="(item, index) in data" :key="index">
                            ...
                            <td>{{FormatCurrency(item.totalOriginalAmount)}}</td>
                            <!-- <td>
                                <div class="d-flex gap-1">
                                    ...
                                    <button type="button" class="btn btn-danger bg-gradient btn-sm p-1" @click="$refs.Delete_Modal._Init({id : item.bizDocId})"
        ...
        SyncData(data){
            ...
            if(this.filters.request.start_time && this.filters.request.end_time)
            {
                ...
                // data ? params = [{"code" : data.id}] : params = [{"code" : ""}]
                    ...
                    let config = {headers: {Authorization: `Bearer ${this.authorize.token}`}}
                    axios.post(PATH_API_WEBHOOK + '/contract-item/sync-data-bravo',params,config)
                    .then(res => {
                        setTimeout(() => {
