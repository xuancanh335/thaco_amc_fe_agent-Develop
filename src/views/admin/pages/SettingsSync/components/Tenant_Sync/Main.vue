<template lang="">
    <div class="tab-pane h-100 fade show active" id="tenant-sync-pane" role="tabpanel">
                ...
                <table class="table table-striped align-middle">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" width="5%">Mã đơn vị</th>
                            <th scope="col" width="20%">Tên đơn vị</th>
                            <th scope="col" width="15%">Tên quản trị</th>
                    ...
                    <tbody v-if="$CheckPermission('Sync_Bravo_Config','is_allow_access')">
                        <tr v-for="(item, index) in data" :key="index">
                            ...
                            <td>{{item.tenDonVi}}</td>
                            <td>{{item.tenQuanTri}}</td>
                            <td>{{item.diaChi}}</td>
                            <td>{{item.maSoThue}}</td>
                            <td>
                                <div class="d-flex gap-1">
                                    <button type="button" class="btn bg-gradient-green btn-sm p-1" 
                                    @click="SyncData({id:item.maDonVi,index:index})"
                                    v-if="$CheckPermission('Sync_Bravo_Config','is_allow_create')"
                                    >
...
export default {
    ...
    methods : {
        ...
        GetDataSync(){
            ...
            axios.post(PATH_API_WEBHOOK + '/tenant-bravo/get-all',params,config)
            .then(res => {
        ...
        SyncData(data){
            ...
            let config = {headers: {Authorization: `Bearer ${this.authorize.token}`}}
            axios.post(PATH_API_WEBHOOK + '/tenant-bravo/sync-data-bravo',params,config)
            .then(res => {
                setTimeout(() => {
