    ...
    <div class="accordion" :id="slug+'Parent'">
        <div class="accordion-item mb-3 rounded-0 border-bottom position-relative">
            <div class="position-absolute end-0 top-0 p-3" style="margin-top:2px">
                <button v-if="!view" @click="$refs.Create_Modal.Display(true,{id:TenantId})" class="btn bg-success rounded-circle p-2 border-0 text-dark hover-jello" href="#">
                ...
                <div class="w-100 border-0 p-3" type="button" data-bs-toggle="collapse" :data-bs-target="'#'+slug" aria-expanded="true">
                    <div class="d-flex">
                        <div class="d-flex align-items-center gap-3 flex-grow-1">
                            ...
                            <div>
                                <h6 class="mb-0 fw-bold text-primary mb-1">Thông tin doanh nghiệp</h6>
                                <p class="mb-0 text-muted" style="font-size:12px">Danh sách thông tin doanh nghiệp</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div :id="slug" class="accordion-collapse collapse " :data-bs-parent="'#'+slug+'Parent'">
                <div class="accordion-body border-top">
                    <table v-if="data && data.length != 0" class="table table-lg table table-striped align-middle">
                        <thead>
                            <tr>
                                <th width="5%" class="text-center py-2">#</th>                           
                                <th class="py-2">Thông tin</th>   
                                <th class="text-capitalize py-2">Diễn giải</th>
                                                             
                                <th class="text-capitalize py-2">Ngày tạo</th>
                                
                                <th class="text-capitalize py-2">Trạng thái</th>
                            ...
                            <tr v-for="(item, index) in data" :key="index">
                                ...
                                <td>
                                    <div class="d-flex gap-1">
                                        <button type="button" @click="$refs.File_Modal.Display(true,item.id)" class="btn btn-primary bg-gradient btn-sm p-1">
                                            <IconPaperclip :size="16" class="d-flex text-white" stroke-width="2" />
                                        </button>
                                        <button v-if="!view" @click="$refs.Update_Modal.Display(true,{id:item.id,tenant_id:TenantId})" type="button" class="btn btn-info bg-gradient btn-sm p-1">
                                            <IconPencil :size="16" class="d-flex text-white" stroke-width="2" />
                                        </button>
                                        <button v-if="!view" @click="$refs.Delete_Modal.Display(true,{id:item.id,tenant_id:TenantId})" type="button" class="btn btn-danger bg-gradient btn-sm p-1">
