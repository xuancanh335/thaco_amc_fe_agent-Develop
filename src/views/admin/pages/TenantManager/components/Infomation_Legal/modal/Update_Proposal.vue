        ...
        <div class="bg-white p-5 rounded-3" style="width:1000px">
            <div class="d-flex mb-4">
                <div class="d-flex align-items-center gap-3 flex-grow-1">
                    ...
                    <div>
                        <h6 class="mb-0 fw-bold text-primary mb-1">Cập nhật thông tin doanh nghiệp</h6>
                        <p class="mb-0 text-muted" style="font-size:12px">Thông tin doanh nghiệp với dữ liệu đi kèm</p>
                    </div>
                </div>
    ...
    methods : {
        async UpdateLegal(){            
            try {
                ...
                axios.post(apiUser + '/tenant-detail/update',params,config)
                .then(async res => {
                    this.Display(false)
        ...
        async GetDataLegal(){            
            try {
                ...
                const params = {
                    item        : this.init.id,
                    tenant_id   : this.init.tenant_id
                }
                const config = {headers: {Authorization: `Bearer ${authorize.token}`}};
                axios.post(apiUser + '/tenant-detail/get-by-id',params,config)
                .then(async res => {
        ...
        async Display(value,data){
            ...
            if(value){
                this.init = await data
                this.$refs.FileManager._Init({id:data.id,type:'TYPE_TENANT_DETAIL'});
