<template lang="">
    <div class="modal fade scale-in-center" id="Modal_DeleteTenant" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                ...
                <div class="modal-body p-4">
                    <div class="d-flex gap-3 mb-4">
                        ...
                        <div class="flex-grow-1">
                            <h5>Xóa đơn vị</h5>
                            <p class="mb-0 text-muted">Bạn sẽ mất tất cả các dữ liệu liên quan đến đơn vị, bạn có chắc chắn muốn xóa !</p>
                        </div>
                    </div>
...
export default {
    ...
    mounted(){
        modal = new bootstrap.Modal('#Modal_DeleteTenant');
    },
    methods : {
        async _Init(value){
        ...
        async DeleteData(){
            try {
                ...
                await axios.post(PATH_API_USER + '/tenant/delete',params,config)
                .then(async res => {
                    this.$emit('RefreshData');
                    modal.hide()
                    ShowToast({status_code : 200, message : 'Xóa đơn vị thành công'});
                })
                .catch(err => {
                    ShowToast({status_code : 401, message : 'Xóa đơn vị thất bại'});
                })
            } catch (error) {
                ShowToast({status_code : 401, message : 'Xóa đơn vị thất bại'});
            }
