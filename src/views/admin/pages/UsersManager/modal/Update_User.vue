<template lang="">
    <div class="modal fade" id="Modal_UpdateUser" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="modal-content">
                <div class="modal-body p-5">
                    <div class="mb-5 d-flex justify-content-between align-items-top">
                        <div class="d-flex gap-3 align-items-center">
                            ...
                            <div>
                                ...
                                <p class="text-muted mb-0">Cập nhật người dùng dựa trên dữ liệu</p>
                            </div>
                        </div>
                        ...
                        <div class="col-lg-5">
                            ...
                            <div class="mb-3">
                                <label for="" class="text-muted mb-2">Số điện thoại</label>
                                    ...
                                    <input type="text" class="form-control shadow-none" v-model="form.phone" placeholder="Nhập số điện thoại..." :mask="'###.###.###-##'" require>
                                </div>
                            </div>
                            ...
                            <div class="card text-left h-100 bg-light" style="max-height: 320px;">
                                <div class="card-header bg-light d-flex justify-content-between align-items-center">
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div class="form-switch d-flex align-items-center gap-1">
                    ...
                    <div class="d-flex justify-content-end gap-2 mt-5">
                        ...
                        <button type="button" class="btn btn-success bg-gradient" @click="UpdateUser">Cập nhật người dùng</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    methods : {
        ...
        async GetDataUser(e){
            ...
            await axios.post(API_USER_PATH + '/user/get-by-id',params,config)
            .then(async res => {
                this.form = res.data.data.userData
                this.tenant_selected = []
                if(res.data.data.userData.is_type_account == 0){
                    this.is_master = true
                    this.tenant_selected.push(this.tenant_master.id)
                    this.tenant_filter.map(tenant => {tenant.is_selected = true})
                    this.tenant.map(tenant => {tenant.is_selected = true})
                }else{
                    this.is_master = false
                    res.data.data.lstTenant.map(item => {
                        if(this.current_user.is_type_account == 0 && this.current_user.is_root_group == 1){
                            this.tenant_selected.push(item.group_id)
                            this.tenant_filter.map(tenant => {
                                if(tenant.id == item.group_id){
                                    tenant.is_selected = true
                                }
                            })
                            this.tenant.map(tenant => {
                                if(tenant.id == item.group_id){
                                    tenant.is_selected = true
                                }
                            })
        ...
        async UpdateUser(){
            if(this.tenant_selected.length >= 1){
                ...
                params.username = this.form.email
                if(this.current_user.is_type_account == 0 && this.current_user.is_root_group == 1){
                    this.is_master ? params.is_type_account = 0 : params.is_type_account = 1
                    params.lstGroup_id = this.tenant_selected
                }
                else{
                    params.is_type_account = 2
                    params.lstTenant_id = this.tenant_selected
                }
                const config = {headers: {Authorization: `Bearer ${this.authorize.token}`}};
                var Modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('Modal_UpdateUser'));
                await axios.post(API_USER_PATH + '/user/update',params,config)
                .then(async res => {
